...
Source: AskJamie scripts/audit-site.py (2026-05-27).

Per-site constants changed from the AskJamie original (everything else
is verbatim):
  - EXPECTED_THEME_COLOR  "#2c5e6f"  →  "#c46a2c"  (OKH rust-orange)
  - EXPECTED_BG_COLOR     "#f5efe1"  →  "#2a2320"  (OKH espresso dark)
  - url_to_relpath()      "https://askjamie.bot"  →  "https://overkillhill.com"
  - og:image check        "https://askjamie.bot"  →  "https://overkillhill.com"
  - render_report header  "AskJamie.bot"  →  "OverKill Hill"
  - Docstring site name   updated throughout

VERIFY before first run:
  1. Open any OKH HTML page and search for name="theme-color".
     Confirm the content value matches EXPECTED_THEME_COLOR (#c46a2c).
     If OKH uses a different value, update the constant here.
...
Per-page checks actually emitted as issues:
  ...
  * theme-color resolves to the OKH brand color (#c46a2c)
...
# OKH rust-orange (#c46a2c) — VERIFY against actual OKH HTML theme-color tags
EXPECTED_THEME_COLOR = "#c46a2c"
# OKH espresso dark (CSS --color-bg: var(--okh-espresso)) — informational, not actively checked
EXPECTED_BG_COLOR = "#2a2320"
...
def audit_page(path: Path) -> List[str]:
    ...
    # og:image existence — when og:image points to overkillhill.com, the
    ...
    for raw_img in re.finditer(r'<img\b[^>]*>', src):
        if "loading=" not in raw_img.group(0):
            src_attr = re.search(r'src="([^"]+)"', raw_img.group(0))
    ...
    # the brand color to appear in at least one tag.
...
def render_report(per_page: Dict[str, List[str]],
    ...
    lines = [
        "# OverKill Hill — Automated Site Audit",
        "",
