...
- **Dynamic search results**: Confirm `aria-live` or `role="status"` is present on the results container to announce result count to screen readers. The current modal uses `role="list"` on `.okh-search-results` but no `aria-live` region announces when new results appear. This is a follow-up item.
