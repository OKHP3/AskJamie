...
Open `scripts/audit-site.py`. Locate the `EXCLUDE_DIRS` (or equivalent) list.
...
## Step 9 — Create `assets/docs/`

Create the directory `assets/docs/` under `assets/`. This folder holds
generated documentation assets — image usage reports, cross-reference files,
and any other non-page, non-template reference documents produced by the
scripts. It is not served as web content.
...
Scripts live in `scripts/` (not `tools/`). Documentation assets live in
`assets/docs/`.
