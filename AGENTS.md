...
#### 2.2.1 Per-site directory inventory (AskJamie)
...
| `assets/css/` | `theme.css` (112 KB) | Single canonical stylesheet |
...
| `assets/docs/` | 8 Markdown audit docs + `sister-site-sync/` subfolder | Includes `sister-site-sync.md` (cross-site constant map), `audit-standards-report.md`, portfolio and image audits |
| `assets/img/` | 76 PNG brand images + 5 subdirs | Full hero and section image set |
| `assets/img/favicons/` | 9 files including `favicon.svg` and `favicon.webp` | Most complete favicon set across siblings |
...
| `assets/js/` | `app.js` (44 KB, unified cross-brand), `mermaid-init.js` | `app.js` is the canonical unified version shared across all three brands |
| `assets/templates/` | 11 templates + index + system prompt | Includes `template--lens-detail.html`, `template--case-study.html`, `template-system-prompt.md` |
| `docs/` | `.gitkeep` only | Add project-level planning docs here |
| `docs/archive/` | `.gitkeep` only | Add archived sprint docs here |
| `scripts/` | 55 scripts | Full shared toolchain |

**AskJamie-specific sub-folders under `assets/img/`:**
- `assets/img/og/` -- Open Graph card images (AskJamie convention). Adopt this
  sub-folder on sibling repos when dedicated OG images are produced.
- `assets/img/brandguard/` -- AI/design reference brand assets. Adopt on siblings
  when brand governance assets are produced for those sites.

**AskJamie-specific resources:**
- `assets/docs/sister-site-sync.md` -- canonical cross-site constant map
  (GA4 IDs, theme colors, localStorage keys, script constants per repo).
  Reference this before adapting any shared script for a sibling site.
- `assets/docs/sister-site-sync/` -- sub-folder for cross-site coordination files.
- `assets/templates/template-system-prompt.md` -- AI system prompt for using
  the template library. Adopt on siblings if useful.
- `assets/templates/template--lens-detail.html` -- AskJamie's lens-system page
  type (equivalent to Glee's `template--tool-detail.html`).
