...
```
Each site also has its own unique content directories (e.g. `lens-system/`
for AskJamie) that are not part of the shared standard. Do not remove content
...
required shared folder. Use this as the decision guide when creating a new
file -- place it in the most specific folder that matches its type. Do not
store working artifacts here (see Section 3).

**`assets/`**
Top-level container for all compiled, generated, and static front-end assets.
HTML pages live at the repo root or in content subdirectories, not here.
Tooling scripts belong in `scripts/`, not here.

**`assets/audit/`**
Machine-generated QA and test-run output worth keeping as a dated record.
Expected files: JSON result files from `scripts/responsive-qa.mjs` (viewport
QA batch runs) and output from `scripts/viewport-qa.py` and
`scripts/run-viewport-qa.py`. Name files with a date suffix, e.g.
`viewport-qa-batch-b1-2026-05-28.json`. Human-authored audit reports belong
in `assets/docs/`, not here.

**`assets/css/`**
Stylesheets only. The canonical shared stylesheet is `theme.css`, which
contains all three brand tiers (GLOBAL, OKH, GLEE, ASKJAMIE). Additional
site-specific CSS files may live here. No JavaScript, no data files, no
images. All filenames must be kebab-case.

**`assets/data/`**
Structured JSON (or YAML) data files consumed by the site at runtime or by
scripts at build time. Expected: `search-index.json` (generated by
`scripts/build-search-index.py`), `icon-map.json` (GPT/tool icon registry),
and other machine-generated or maintained data files used by pages or scripts.
Audit output and documentation do not belong here.

**`assets/downloads/`**
Files offered for direct download by site visitors: PDFs, ZIPs, exportable
CSVs, printable guides, etc. Keep this minimal. Use `.gitkeep` when empty.
Internal tooling assets and page images do not belong here.

**`assets/docs/`**
Human-authored project documentation, audit reports, design records, planning
artifacts, and cross-site sync guides. All files must be Markdown (`.md`)
with kebab-case names. Expected naming patterns:
`audit-report-YYYY-MM-DD.md`, `AUDIT-TOPIC-YYYY-MM-DD.md`,
`og-image-requirements.md`, `sister-site-sync.md`.
Machine-generated JSON test output belongs in `assets/audit/`, not here.
Superseded docs move to `docs/archive/` rather than being deleted.

**`assets/img/`**
All site images used directly in HTML pages. Filenames must be kebab-case
with descriptive slugs and dimension or variant suffixes where applicable,
e.g. `brand-hero-wide-1536.png`, `jamie-avatar-square-1024.png`.
WebP conversions live in `assets/img/webp/`. Unused brand library assets
live in `assets/img/library/`. Favicons live in `assets/img/favicons/`.

**`assets/img/favicons/`**
The complete favicon set for the site. Expected files: `favicon.ico`,
`favicon-16x16.png`, `favicon-32x32.png`, `favicon-48x48.png`,
`favicon.svg`, `android-chrome-192x192.png`, `android-chrome-512x512.png`,
`apple-touch-icon.png`. Generated once; rarely changed. Referenced via
`<link rel="icon">` tags in every page `<head>`.

**`assets/img/library/`**
Extended brand image library: unused variants, alternate crops, source assets,
and color-variant PNGs kept for reference or future use. Files here are not
necessarily referenced by any live page -- this is the on-disk brand archive
so assets are not permanently lost. Do not store WebP conversions here.

**`assets/img/webp/`**
WebP conversions of PNG and JPG assets. Generated by `scripts/png-to-webp.py`
or `scripts/picture-upgrade.py`. Filenames mirror their source with a `.webp`
extension and optional size suffix, e.g. `brand-hero-wide-1536.webp`,
`gpt-icon-01-careers-retro-stripe-512.webp`. Originals stay in `assets/img/`.

**`assets/js/`**
JavaScript files served to browsers. The canonical shared files are `app.js`
(site interactivity: search, nav, GA4 events, scroll reveal) and
`mermaid-init.js` (Mermaid v11 ESM initialization). No build step; files are
served directly as-is. No TypeScript, no bundled output. Node.js tooling
scripts belong in `scripts/`, not here.

**`assets/templates/`**
Reusable HTML page-shell fragments used to scaffold new pages. Must include
an `INDEX.md` cataloging every template's filename, purpose, and required
placeholder variables. Template files use a `template--` prefix, e.g.
`template--interior-single.html`, `template--hub-branch.html`.
Finished, published pages do not belong here.

**`docs/`**
Top-level documentation directory rendered by GitHub as the project's
documentation root. Intended for project-level documentation accessible
directly from the repo homepage -- keep separate from `assets/docs/`, which
holds internal audit and planning artifacts. Use `.gitkeep` when empty.

**`docs/archive/`**
Archived and superseded documentation removed from active use. Move files
here rather than deleting them when they are no longer current but may be
referenced historically. Use `.gitkeep` when empty.
...
kebab-case. Scripts are run manually or invoked from `post-merge.sh`; they
are never served to browsers. When distributing a script to a sibling repo,
adapt per-site constants: `EXPECTED_THEME_COLOR`, `SITE_URL`/`SITE_ORIGIN`,
and `GA4_ID`. See `assets/docs/sister-site-sync.md` for the full constant
map and distribution checklist.
...
### 6. Brand contract (AskJamie)

This repo serves the AskJamie brand.
Canonical reference: https://raw.githubusercontent.com/OKHP3/OverKill-Hill/main/assets/css/theme.css
(scope `.askjamie-main`)

AskJamie motif declared values:

| Aspect | Value |
|---|---|
| Body scope class | `askjamie-main` (pages MUST set `<body class="askjamie-main">`) |
| Heading font | Baloo 2 |
| Body font | Open Sans |
| Accent / handwritten | Kalam |
| Mono font | JetBrains Mono |
| Page background | paper `#f6f2ee` |
| Card surface | `#fdfbf7` |
| Text / ink | charcoal `#2e2b29` |
| Primary accent | teal `#2d6f7e` |
| Footer background | `#f7f3ee` (softened; overrides OverKill's dark footer) |
| Mermaid line/border | teal `#2d6f7e` |
| Tone | calm, mid-century, conversational |

**Forbidden in this brand's apps:**
- Rust-orange `#c46a2c` (that is OverKill Hill P3)
- Coral `#d94f63` (that is Glee-fully)
- Forge espresso/teal-dark palette or Alfa Slab One (that is OverKill Hill P3)
- Fredoka headings (that is Glee-fully)
- Industrial / forge aesthetic, blueprint grids, slab serifs
- Dark mode by default (this brand is paper/light-first)
