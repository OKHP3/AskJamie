---
name: okhp3-translation-en-us-es-es
description: >
  Translate owned plainspoken text artifacts one way from United States English
  (en-US) to Spain Spanish (es-ES), preserving source voice, meaning,
...
metadata:
  ...
  in_scope: "Directed en-US to es-ES text-artifact translation, voice preservation, pair dictionary enforcement, provenance, parity checks, and review gating."
...
# okhp3-translation-en-us-es-es

**OverKill Hill P³** · [overkillhill.com](https://overkillhill.com) · [github.com/OKHP3](https://github.com/OKHP3)

Create one derived Spain Spanish text artifact from one authoritative,
...
|---|---|
| UTF-8 HTML, Markdown, MDX, plain text, and text-resource artifacts already in plainspoken en-US | es-419, es-MX, es-US, French, German, or any other language pair |
| Source voice, Spain Spanish register, pair dictionary, metadata, and protected-token parity | Rewriting the en-US source or inventing a Spanish author persona |
...
```text
owned plainspoken en-US text artifact -> machine-drafted plainspoken es-ES text artifact
```

The en-US artifact is always the source of truth. Do not use an older Spanish
...
warmth, humor, and deliberate emphasis in natural Spanish. Do not polish the
...
- one owned, plainspoken `en-US` source artifact and one separate `es-ES`
  target path;
- an artifact type: `html`, `markdown`, `mdx`, `plain-text`, `text-resource`,
  or `adapter-required`;
- an owner-approved source voice profile;
- `assets/dictionary.en-us-es-es.json`, copied to the consuming project and
  extended only with reviewed project terms; and
- the single-pair manifest based on
  `assets/translation-project.en-us-es-es.example.json`.

If the source is not demonstrably plainspoken `en-US`, the target is not
`es-ES`, the artifact needs a missing format adapter, or a required pair record
...
vague "Spanish" or "English" to proceed.
...
3. Read the en-US voice profile and en-US to es-ES dictionary. Owner-approved
...
5. Translate each semantic unit using the declared treatment:
   ...
   - `transcreate` for headings, slogans, and calls to action that must work in
     Spain Spanish; and
   - `adapt` for Spanish SEO titles, descriptions, or other target-audience
     discovery fields.
6. Apply the pair dictionary exactly. Preserve brands, product names, code,
   ...
   Translate reader-facing labels, metadata, and alt text. Apply Spain Spanish
   language conventions only as the pair dictionary or project record directs.
7. Write the es-ES target and a translation record with paths, hashes, source
...
es-ES draft task. It must use the source hash, apply this pair's dictionary,
preserve reviewed unchanged Spanish units, retain a diff, and stop at draft
...
This package does not create French or German copies, and it does not create
Latin American Spanish (`es-419`), Mexican Spanish (`es-MX`), or United States
Spanish (`es-US`) copies. Create a separately named pair package only after its
exact target locale, audience policy, and dictionary are chosen. The adjacent
naming pattern is `okhp3-translation-<source-locale>-<target-locale>`.

## Quality and review gates

Before handoff, verify that source propositions, required metadata, links,
protected tokens, and intentional omissions are recorded; Spanish reads naturally
...
agreement, back-translation, or a dictionary match cannot certify native Spanish.

## Output contract

Return `Source artifact`, `Spain Spanish target artifact`, `Pair record`,
`Voice and dictionary`, `Translation notes`, `Validation`, `Review gate`, and
`Next action`. State the actual direction in every handoff: `en-US -> es-ES`.

## Resource routing

- Read `references/en-us-to-es-es-contract.md` for records, protected regions,
  incremental updates, and publication boundaries.
- Read `references/voice-and-spanish.md` for source-voice evidence, Spain
  Spanish register, typography, and dictionary precedence.
...
-p "test_*.py" -v`. The discoverable `tests/test_en_us_to_es_es.py` suite
...
`tests/test-en-us-to-es-es.py` implementation was consolidated into that
