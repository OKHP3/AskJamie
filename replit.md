...
`scripts/audit-site.py` — the auditor flags any Mermaid page missing
...
- **Index:** `assets/data/search-index.json` — generated by `scripts/build-search-index.py` from every `.html` file (excluding `404.html` and `under-construction.html`). Each record stores: url, title, description, section label, h1, h2/h3 list, body excerpt (capped at 4000 chars per page).
...
- **Rebuilding:** run `python3 scripts/build-search-index.py` after any HTML/copy change.
...
- **Search index bugfix** — `scripts/build-search-index.py` was producing
...
`scripts/enhance-pages.py` is the new idempotent bulk-edit tool that
