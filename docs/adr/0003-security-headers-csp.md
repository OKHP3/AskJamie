...
The presence of `_headers` does not establish enforcement by GitHub Pages. Treat this file as portable host configuration and verify production HTTP responses before claiming clickjacking or other edge controls. Current page CSP is generated by `scripts/csp.py` and `scripts/generate-csp.py`, with policy classes and exact script hashes. The original decision above is retained as historical rationale; its assertion that the current CDN applies `_headers` is not current verified evidence.
